<template>
  <div class="grid grid-cols-4">
    <div class="hidden md:flex relative col-span-1 items-center justify-center">
      <ElementsImageBlur src="/quote.png" alt="Quote" position="relative" width="150" />
    </div>
    <swiper class="h-52 pr-5 md:pl-14 md:pr-32 overflow-hidden col-span-full md:col-span-3" :modules="modules"
      :speed="1000" :space-between="50" :loop="true" :direction="'vertical'" :pagination="{ clickable: true }" :autoplay="{
        delay: 7000,
        disableOnInteraction: false,
        pauseOnMouseEnter: true,
      }" :slides-per-view="1" @swiper="onSwiper" @slideChange="onSlideChange">
      <swiper-slide v-for="(text, index) in storeTestimonials.testimonials" :key="index">
        <div>
          <p class="text-white dark:text-primary text-md md:text-xl mb-5">
            {{ text.content }}
          </p>
          <span class="text-grey-dark dark:text-grey-light uppercase font-arimo tracking-wide italic">{{ text.title
          }}</span>
        </div>
      </swiper-slide>
    </swiper>
